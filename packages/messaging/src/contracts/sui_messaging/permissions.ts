...
export interface NewRoleOptions {
  package?: string;
  arguments: NewRoleArguments | [permissions: RawTransactionArgument<string>];
}
...
export interface PermissionsOptions {
  package?: string;
  arguments: PermissionsArguments | [self: RawTransactionArgument<string>];
}
export function permissions(options: PermissionsOptions) {
  const packageAddress = options.package ?? '@local-pkg/sui_messaging';
  const argumentsTypes = [`${packageAddress}::permissions::Role`] satisfies string[];
...
export interface HasPermissionOptions {
  ...
  arguments:
    ...
    | [self: RawTransactionArgument<string>, permission: RawTransactionArgument<string>];
}
